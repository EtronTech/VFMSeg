...
class Net2DSeg(nn.Module):
    ...
    def forward(self, data_batch,mixed_batch=None):
        ...
        if mixed_batch is None:
            ...
            img = mixed_batch[0] # using SAM Augmented Data
            img_indices = mixed_batch[1]
        ...
        for i in range(x.shape[0]):
            ...
            # img_org_feats.append(x.permute(0, 2, 3, 1)[i][:,:].flatten(start_dim=0,end_dim=1))  # For Validation
        img_feats = torch.cat(img_feats, 0)
        
        # img_org_feats = torch.cat(img_org_feats,0) # For Validation
      

        # linear
        x = self.linear(img_feats)
        # x_org = self.linear(img_org_feats) # For Validation

        preds = {
            'feats': img_feats,
            'seg_logit': x #,
            # 'seg_logit_orig': x_org,
        }
