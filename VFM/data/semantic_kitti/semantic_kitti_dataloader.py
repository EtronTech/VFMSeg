...
class SemanticKITTISCN(SemanticKITTIBase):
    def __init__(self,
        ...
        self.vfm_data_paths = vfm_data_paths + '/' + split[-1]
    ...
    def __getitem__(self, index):
        ...
        sampled_sam_mask,sampled_sam_indices = sample_mix_masks(masks_sam, out_dict['img_indices'])
        cut_mix_mask,cut_mix_indices = get_cut_masks(out_dict['img'], out_dict['img_indices'])


        if self.pselab_data is not None:
            out_dict['pseudo_label_2d'] = torch.from_numpy(self.pselab_data[index]['pseudo_label_2d'][keep_idx][idxs])
            ...
            if self.pselab_data[index]['pseudo_label_3d'] is None:
                ...
                out_dict['pseudo_label_3d'] = torch.from_numpy(self.pselab_data[index]['pseudo_label_3d'][keep_idx][idxs])
